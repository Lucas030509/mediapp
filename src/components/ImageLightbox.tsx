        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-sm">
            ...
            <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center bg-gradient-to-b from-black/60 to-transparent z-10 text-white">
                ...
                <div className="flex gap-2 bg-slate-900/60 p-1.5 rounded-2xl border border-white/10 backdrop-blur-md">
                    <button onClick={() => setScale(s => Math.min(s + 0.2, 5))} className="p-2 hover:bg-white/20 rounded-xl transition" title="Acercar">
                        <ZoomIn className="w-5 h-5" />
                    </button>
                    <button onClick={() => setScale(s => Math.max(s - 0.2, 0.5))} className="p-2 hover:bg-white/20 rounded-xl transition" title="Alejar">
                        <ZoomOut className="w-5 h-5" />
                    </button>
                    <div className="w-px h-6 bg-white/20 mx-1 self-center" />
                    <button onClick={() => setRotation(r => r - 90)} className="p-2 hover:bg-white/20 rounded-xl transition" title="Reotación Izquierda">
                        <RotateCcw className="w-5 h-5" />
                    </button>
                    <button onClick={() => setRotation(r => r + 90)} className="p-2 hover:bg-white/20 rounded-xl transition" title="Rotación Derecha">
                    ...
                    {onToggleFavorite && (
                        <>
                            <div className="w-px h-6 bg-white/20 mx-1 self-center" />
                            <button onClick={onToggleFavorite} className={`p-2 rounded-xl transition flex items-center gap-2 px-4 ${isFavorite ? 'bg-amber-500/20 text-amber-400 font-bold' : 'hover:bg-white/20 text-white'}`} title="Marcar Caso de Éxito / Top 1">
